...
- [~] P3 — host/Platform + IMod + Commander as first mod (single plugin initially; see specs/phase-3/P3-host.md) **PT**
  ...
  - [~] P3d — loader UI built: `src/Host/MainMenuLoader` adds a MODS main-menu button → panel listing registered mods (from ModRegistry) with per-mod ON/OFF toggles (runtime SetEnabled). MainMenuBadge.FindMenuCanvas made public for reuse. Emits [NUCLEUS:SELFTEST] loader-ui-built. Build+7 gates green, deployed. **Playtest queued: playtests/P3d-loader.md.** Follow-up: persist enabled-state to BepInEx config; layout polish. **PT**
  - [ ] P3-integration — `Nucleus.TestKit` FakeGame + `Nucleus.Integration.Tests` (lifecycle, slots, shared queue, no double-buy) **HV**
- [~] P4 — Build as its OWN plugin (apps/Nucleus.Build, GUID com.nucleus.build): registers via ModPlatform pending-buffer (order-independent; soft-dep on platform GUID forward-compat), gets injected IGameServices across the plugin boundary, emits [NUCLEUS:SELFTEST] build-mod-loaded + buildFunds metric. **No-skew deploy verified** (only Nucleus.Build.dll in its folder; shared libs Private=false, ship once with the host). Build + 7 gates green (arch inspects it as an app). **First separate plugin — platform extensibility proven.** Playtest auto-verifies via self-test. Follow-up: real buy-menu UI once host UI/button services land. **PT**
- [~] P5 — Squad as its OWN plugin (apps/Nucleus.Squad, GUID com.nucleus.squad): same proven pattern as P4 — registers via buffer, reads shared roster (ctx.Game.Roster()) across the boundary, emits [NUCLEUS:SELFTEST] squad-mod-loaded + squadRoster metric. No-skew deploy verified. Build+7 gates green (3 apps now). Playtest auto-verifies. Follow-up: real squad UI once host UI/button services land. **PT**
